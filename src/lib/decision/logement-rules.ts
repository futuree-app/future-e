...
function coverageRule(cfg: {
  ...
  return {
    ...
    evaluate: (f, p): RuleEvaluation => {
      ...
      if (cov === "unavailable") return out(cfg.id, logementScopedUnknown(cfg.id, ev(l, `logement.${cfg.id}`, "live_fetch", grain), cfg.unavailableStatement));
      if (cov === "present" && cfg.flag(l)) {
        return out(cfg.id, logementVerification(cfg.id, ev(l, `logement.${cfg.id}`, "live_fetch", grain, cfg.observedValue?.(l)), cfg.tier, cfg.statement(l), cfg.actionType, cfg.action[b], cfg.limitation));
      }
      return na(cfg.id);
    },
  };
...
const ruleDpe: DecisionRule = {
  ...
  evaluate: (f, p): RuleEvaluation => {
    ...
    return out("dpe-faible", logementVerification("dpe-faible", evidence, "structuring", `À cette adresse, le diagnostic choisi classe ce logement ${cls}.`, "demander_confirmation", action[bucket(p)]));
  },
};
...
export const LOGEMENT_RULES: DecisionRule[] = [
  ...
  coverageRule({ id: "sinistralite", tier: "secondary", grain: "commune", coverage: (l) => l.sinistralite, flag: (l) => l.sinistraliteActive,
