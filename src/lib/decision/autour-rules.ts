 ...
 *
 * Appeler un cabinet avant de s'engager n'a de sens que pour qui s'engage. Quelqu'un qui habite
...
const GESTE_SOINS: Record<Bucket, { label: string; detail: string }> = {
  achat: {
    label: "Appelez ce cabinet avant de vous engager",
    detail:
      "La saturation locale ne se lit dans aucune base : un cabinet proche peut être fermé aux nouveaux patients depuis des années.",
  },
  location: {
    label: "Vérifiez qu'un médecin vous accepte avant de signer",
    detail:
      "La présence d'un cabinet ne dit rien de sa disponibilité. Un appel suffit à le savoir.",
  },
  reside: {
    label: "Vérifiez que ce cabinet prend encore de nouveaux patients",
    detail:
      "Utile avant d'en avoir besoin : la disponibilité change, et elle ne se lit dans aucune base.",
  },
  neutre: {
    label: "Vérifiez la disponibilité réelle de ce cabinet",
    detail:
...
function constatSante(e: EquipementProche): string {
  ...
  const choix =
    e.lieuxAPortee > 1
      ...
      : "";
  return `Autour de cette adresse, ${quoi}${nomme} est recensé ${ou}.${choix}`;
}
...
const accesSoinsRule: DecisionRule = {
  ...
  evaluate: (f, p): RuleEvaluation => {
    ...
    // soins : le cabinet peut être à 3 km, hors du rayon cherché.
    if (equipements.sante === null) {
      const fact: VerificationFact = {
        ...
        limitation:
          "Le périmètre de recherche est borné : un cabinet situé au-delà n'apparaît pas ici, et la commune peut rester bien dotée.",
        evidence: [
          {
            factId: "autour.sante",
            module: "logement",
        ...
        action: {
          ...
          label: "Repérez le cabinet le plus proche avant de décider",
