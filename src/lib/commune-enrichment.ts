// ─── Enrichissement multi-sources (ADEME, DRIAS, Hub'Eau) ──────────────────
// Les trois sources sont fetchées en parallèle. Chaque source peut échouer
...
export async function gatherCommuneEnrichment(
  ...
  const [ademeRes, driasRes, eauRes] = await Promise.allSettled([
